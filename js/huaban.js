...
var canvasTop = document.documentElement.clientWidth/850*100*4; 
...
blue.onclick = function(){
	selectColor = true;
  context.fillStyle = '#a75558';
  context.strokeStyle = '#a75558';
...
green.onclick = function(){
	selectColor = true;
  context.fillStyle = '#6085a5';
  context.strokeStyle = '#6085a5';
...
yellow.onclick = function(){
	selectColor = true;
  context.fillStyle = '#edb63c';
  context.strokeStyle = '#edb63c';
...
purple.onclick = function(){
	selectColor = true;
  context.fillStyle = '#d76755';
  context.strokeStyle = '#d76755';
...
function getPercentage(){
	yyoyu.webapi.subscribe.getPercentage(function(data) {
		console.log('getPercentage='+JSON.stringify(data));
		if(data.code == 200){
			var percentage = data.data.percentage;
			xishu = parseFloat(percentage);
			version = data.data.version;
			localStorage.setItem('hnjh_huayy_version', version);
			console.log('percentage='+percentage+ ' xishu='+xishu+' version='+version);
		}else{
...
function postZuoPin(){
	...
	yyoyu.webapi.subscribe.postZuoPin(params, function(data) {
		console.log('postZuoPin='+JSON.stringify(data));
		if(data.code == 200){
			//提交作品成功，把当前作品的id存起来
			//本地只保存一个作品id
			localStorage.setItem('zuopin_id', id);
		}else{
			//app.showToast(data.msg);
		}
	}, function(xhr, type, errorThrown) {
